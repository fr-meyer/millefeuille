"""Offline one-use ledger checks for an exact GPT summary output write."""
...
class TestGptSummaryOutputReplayReservation(unittest.TestCase):
    def test_consumes_exact_write_receipt_once_without_paper_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with _trusted_control(control, approval):
                preview = reservation.reserve_trusted_gpt_summary_output_write_receipt(
                    **evidence
                )
                with self.assertRaises(MillefeuilleContractError):
                    reservation.reserve_trusted_gpt_summary_output_write_receipt(
                        **evidence
                    )
            after = {p: p.read_bytes() for p in root.rglob("*") if p.is_file()}
            self.assertEqual(before, after)
            self.assertEqual(preview.files_written, 0)
            database = control / "gpt-summary-write.sqlite3"
    ...
    def test_missing_approval_does_not_create_ledger(self):
        _root, evidence, _record = _case(self)
        with tempfile.TemporaryDirectory() as tempdir:
            control = Path(tempdir) / "control"
            control.mkdir(mode=0o700)
            approval = control / "approval.json"
            with (
                ...
                reservation.reserve_trusted_gpt_summary_output_write_receipt(**evidence)
            self.assertFalse((control / "gpt-summary-write.sqlite3").exists())
