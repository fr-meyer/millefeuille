"""Privileged durable reservation for one approved GPT summary receipt.
...
from millefeuille.domain.summary_trusted_approval import (
    validate_trusted_gpt_summary_approval,
)

_CONTROL_DIR = Path("/etc/millefeuille")
_CONTROL_OWNER_UID = 0
_LEDGER_NAME = "gpt-summary.sqlite3"


def reserve_trusted_gpt_summary_receipt(
    *,
    ...
    if sys.platform != "linux" or os.geteuid() != 0:
        raise MillefeuilleContractError("GPT summary reservation requires Linux root")
    _require_control_dir()
    preview = validate_trusted_gpt_summary_approval(
    ...
    if not hasattr(os, "O_NOFOLLOW"):
        raise MillefeuilleContractError("GPT summary ledger requires no-follow support")
    try:
        descriptor = os.open(
            database,
        ...
        raise MillefeuilleContractError("GPT summary ledger is unavailable") from exc
    else:
        os.close(descriptor)
...
def _require_control_dir() -> None:
    try:
        ...
        raise MillefeuilleContractError(
            "GPT summary administrator control directory is missing"
        ) from exc
    if (
        ...
        raise MillefeuilleContractError(
            "GPT summary control directory is not administrator-owned"
        )
...
def _require_private_database_or_absent(path: Path) -> None:
    ...
    if (
        ...
        raise MillefeuilleContractError(
            "GPT summary replay database is not administrator-owned"
        )
...
def _parse_canonical_audit(value: str) -> dict[str, Any]:
    try:
        ...
        raise MillefeuilleContractError("GPT summary replay ledger is invalid") from exc
    if not isinstance(payload, dict) or _canonical_json(payload) != value:
        raise MillefeuilleContractError("GPT summary replay ledger is not canonical")
    return payload
