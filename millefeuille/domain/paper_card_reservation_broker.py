"""One-shot root broker for an exact GPT card receipt reservation.
...
from millefeuille.domain.paper_card_replay_reservation import (
    _CONTROL_DIR,
    _require_control_dir,
    reserve_trusted_gpt_card_receipt,
)
...
_BROKER_SOCKET_PATH = Path("/etc/millefeuille/gpt-card.sock")
_BROKER_OWNER_UID = 0
...
def request_gpt_card_reservation(
    *,
    ...
    preview = validate_gpt_card_approval_preview(
        **evidence,
    ...
    try:
        ...
        raise MillefeuilleContractError("GPT card broker is unavailable") from exc
    if (
        os.name != "posix"
        ...
        raise MillefeuilleContractError("GPT card broker is not administrator-owned")
    ...
    if len(request_bytes) > _REQUEST_LIMIT:
        raise MillefeuilleContractError("GPT card broker request is too large")
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as channel:
            ...
            if size > _REPLY_LIMIT:
                raise MillefeuilleContractError("GPT card broker reply is too large")
            reply = json.loads(
        ...
        raise MillefeuilleContractError("GPT card broker failed closed") from exc
    if (
        ...
        raise MillefeuilleContractError("GPT card broker refused reservation")
    return preview
...
def serve_one_reservation(*, timeout_seconds: int = 120) -> None:
    ...
    if (
        ...
        raise MillefeuilleContractError("GPT card broker requires Linux root")
    _require_control_dir()
    if _BROKER_SOCKET_PATH.parent != _CONTROL_DIR:
        raise MillefeuilleContractError("GPT card broker socket path drifted")
    if _BROKER_SOCKET_PATH.exists() or _BROKER_SOCKET_PATH.is_symlink():
        raise MillefeuilleContractError("GPT card broker socket already exists")
    import pwd
    ...
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as listener:
            ...
            while True:
                ...
                if remaining <= 0:
                    raise TimeoutError("GPT card broker listener timed out")
                ...
                try:
                    ...
                    if size > _REQUEST_LIMIT:
                        raise MillefeuilleContractError(
                            "GPT card broker request is too large"
                        )
                    payload = json.loads(
                        _read_exact(channel, size).decode("utf-8"),
                        object_pairs_hook=_unique_json_object,
...
def _process_request(payload: Any) -> dict[str, Any]:
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise MillefeuilleContractError("GPT card broker request is invalid")
    ...
    if (
        ...
        raise MillefeuilleContractError("GPT card evidence paths are invalid")
    if not isinstance(payload["publication"], dict):
        raise MillefeuilleContractError("GPT card published identity is invalid")
    try:
        publication = GptSummaryPublicationIdentity(**payload["publication"])
    except TypeError as exc:
        raise MillefeuilleContractError(
            "GPT card published identity is invalid"
        ) from exc
    ...
    preview = reserve_trusted_gpt_card_receipt(
        **evidence,
...
def _require_evidence_paths(evidence: dict[str, str], source_pack_root: str) -> None:
    ...
    for value in evidence.values():
        ...
        read_bytes_no_follow(path, "GPT card evidence")


def _require_peer_uid(channel: socket.socket, expected_uid: int) -> None:
    if not hasattr(socket, "SO_PEERCRED"):
        raise MillefeuilleContractError("GPT card broker needs peer credentials")
    identity = channel.getsockopt(socket.SOL_SOCKET, socket.SO_PEERCRED, 12)
    _pid, uid, _gid = struct.unpack("3i", identity)
    if uid != expected_uid:
        raise MillefeuilleContractError("GPT card broker peer identity mismatch")
...
def _read_exact(channel: socket.socket, size: int) -> bytes:
    ...
    while len(result) < size:
        ...
        if not part:
            raise OSError("GPT card broker closed early")
        result.extend(part)
    return bytes(result)
