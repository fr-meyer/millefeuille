...
def _validate_structure_payload(
    ...
    if (
        type(coverage.get("expected_pages")) is not int
        or coverage["expected_pages"] != evidence.page_count
    ):
        raise MillefeuilleContractError(
            "summary preparation structure expected page coverage drift"
        )
    detected_pages = coverage.get("detected_pages")
    if type(detected_pages) is not int or detected_pages < 0:
        raise MillefeuilleContractError(
    ...
    if coverage.get("locators") != sum(collection_counts.values()):
        raise MillefeuilleContractError(
            "summary preparation structure locator coverage drift"
        )
