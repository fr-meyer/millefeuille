"""Root-only GPT card publication from a validated transient handoff.
...
from millefeuille.domain.paper_card_outcome_handoff import (
    decode_gpt_card_outcome_handoff,
)
from millefeuille.domain.paper_card_output_replay_reservation import (
    ...
    reserve_trusted_gpt_card_output_write_receipt,
)
...
class GptCardPublicationAuditUncertainError(MillefeuilleContractError):
    """The receipt is consumed and the terminal audit state is unknown."""


def publish_trusted_gpt_card_handoff(
    *,
    encoded_outcome: bytes,
    execution_approval: GptCardApprovalPreview,
    ...
    outcome = decode_gpt_card_outcome_handoff(
        encoded_outcome,
        expected_approval=execution_approval,
    ...
    bundle = plan_gpt_card_publication_bundle(**scope)
    reserved = reserve_trusted_gpt_card_output_write_receipt(
    ...
    try:
        fresh = plan_gpt_card_publication_bundle(**scope)
        if fresh != bundle:
            raise MillefeuilleContractError("GPT publication source or output drift")
        committed = commit_prevalidated_gpt_card_bundle(
            bundle=fresh, source_pack_root=source_pack_root
        )
    except Exception as exc:
        uncertain = isinstance(exc, GptCardPublicationError) and (
        ...
        try:
            ...
            raise GptCardPublicationAuditUncertainError(
    ...
    try:
        ...
        raise GptCardPublicationError(committed=True, cleanup_complete=True) from exc
    return committed
