"""Read-only administrator approval check for one grounded GPT card batch.
...
_TRUSTED_APPROVAL_PATH = Path("/etc/millefeuille/gpt-card-approval.json")
_TRUSTED_APPROVAL_OWNER_UID = 0
_TRUSTED_APPROVAL_SCHEMA = "millefeuille-gpt-card-approval/v0.1"
...
def validate_trusted_gpt_card_approval(
    *,
    ...
    try:
        ...
        raise MillefeuilleContractError("GPT card approval record is invalid") from exc
    if (
        ...
        raise MillefeuilleContractError("GPT card approval identity does not match")
    return preview
...
def _read_administrator_file(path: Path) -> bytes:
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                raise MillefeuilleContractError(
                    "GPT card approval directory is not administrator-owned"
                )
            ...
            try:
                ...
                if (
                    ...
                    raise MillefeuilleContractError(
                        "GPT card approval file is not administrator-owned"
                    )
        ...
        raise MillefeuilleContractError(
            "GPT card has no readable administrator approval"
        ) from exc
    if len(encoded) > _RECORD_LIMIT:
        raise MillefeuilleContractError("GPT card approval record is too large")
    return encoded
