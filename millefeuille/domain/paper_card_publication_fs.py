...
from millefeuille.domain.paper_card_output_plan import PlannedGptCardFile
...
class GptCardPublicationError(MillefeuilleContractError):
...
def commit_prevalidated_gpt_card_bundle(
    *, bundle: GptCardPublicationBundle, source_pack_root: str | Path
    ...
    stage_created = False
    committed = False
    result: GptCardFilesystemCommit | None = None
    ...
    try:
        ...
        stage_name = ".gpt-card-stage-" + uuid.uuid4().hex
        ...
        result = GptCardFilesystemCommit(
...
def _validate_bundle(
    ...
    if not isinstance(bundle, GptCardPublicationBundle):
        raise MillefeuilleContractError("GPT publication bundle is invalid")
    preview = bundle.preview
    if not isinstance(preview, GptCardOutputWritePreview):
    ...
    if (
        ...
        or len(bundle.files) != 5
        or any(not isinstance(item, PlannedGptCardFile) for item in bundle.files)
        or any(
        ...
        or type(bundle.total_bytes) is not int
        or bundle.total_bytes != preview.total_bytes
    ...
    if (
        ...
        != "millefeuille-gpt-card-publication-bundle/v0.1"
        ...
        or manifest.get("request_plan_sha256") != preview.request_plan_sha256
        or manifest.get("summary_publication_sha256")
        != preview.summary_publication_sha256
        or manifest.get("provenance_sha256") != preview.provenance_sha256
