"""Trusted administrator approval for one exact GPT card output write.
...
_TRUSTED_APPROVAL_PATH = Path("/etc/millefeuille/gpt-card-write-approval.json")
_TRUSTED_APPROVAL_OWNER_UID = 0
_TRUSTED_APPROVAL_SCHEMA = "millefeuille-gpt-card-write-approval/v0.1"
...
def validate_trusted_gpt_card_output_write_approval(
    *,
    ...
    preview = validate_gpt_card_output_write_preview(
        outcome=outcome,
    ...
    try:
        ...
        raise MillefeuilleContractError(
            "GPT card write approval record is invalid"
        ) from exc
    if (
        ...
        or record["total_bytes"] != preview.total_bytes
        ...
        or not _digest_matches(
            record["request_plan_sha256"], preview.request_plan_sha256
        )
        or not _digest_matches(
            record["summary_publication_sha256"], preview.summary_publication_sha256
        )
        or not _digest_matches(record["provenance_sha256"], preview.provenance_sha256)
    ):
        raise MillefeuilleContractError(
            "GPT card write approval identity does not match"
        )
    return preview
...
def _read_administrator_file(path: Path) -> bytes:
    if not all(hasattr(os, name) for name in ("O_DIRECTORY", "O_NOFOLLOW")):
        raise MillefeuilleContractError(
            "GPT card write approval requires no-follow support"
        )
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                raise MillefeuilleContractError(
                    "GPT card write approval directory is not administrator-owned"
                )
            ...
            try:
                ...
                if (
                    ...
                    raise MillefeuilleContractError(
                        "GPT card write approval file is not administrator-owned"
                    )
        ...
        raise MillefeuilleContractError(
            "GPT card has no readable administrator write approval"
        ) from exc
    if len(encoded) > _RECORD_LIMIT:
        raise MillefeuilleContractError("GPT card write approval record is too large")
    return encoded
